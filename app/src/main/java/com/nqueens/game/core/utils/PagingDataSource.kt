...
class PagingDataSource<PARAMS : Any, DOMAIN_MODEL, UI_MODEL>(
    ...
    private val pages = MutableStateFlow<Map<Int, Page<DOMAIN_MODEL>>>(emptyMap())
    ...
    val uiState =
        builder(
            pages.filter { it.isNotEmpty() }.map { pageMap ->
    ...
    private fun load(page: Int) {
        if (pages.value[page] == null) {
            jobs[page]?.cancel()
            jobs[page] =
                loadPage(params, page)
                    .distinctUntilChanged()
                    .onEach { domainModels ->
                        val mutableMap = pages.value.toMutableMap()
                        mutableMap[page] = Page(domainModels)
                        pages.value = mutableMap
