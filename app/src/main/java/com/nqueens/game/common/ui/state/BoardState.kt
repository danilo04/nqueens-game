package com.nqueens.game.common.ui.state
...
abstract class BoardState(private val boardGame: BoardGame) {
    ...
    val boardCells: Map<BoardPosition, Flow<CellState>> = buildMap {
    ...
    fun selectCell(position: BoardPosition, state: SelectedState = SelectedState.TO_MOVE) {
    ...
    fun isCellSelected(position: BoardPosition): Boolean {
        return cellSelections[position]?.value != SelectedState.NOT_SELECTED
    }

    fun setPositionError(position: BoardPosition, hasError: Boolean) {
    ...
    abstract fun tapOnCell(x: Int, y: Int)

    abstract fun resetGame()

    fun getCellState(x: Int, y: Int): Flow<CellState> {
