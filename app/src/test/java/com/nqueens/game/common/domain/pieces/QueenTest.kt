...
class QueenTest : BaseUnitTest() {
    ...
    fun `given a board with enemy pieces, queen can capture but not move beyond them`() {
        ...
        val enemyQueen = Queen(PieceColor.BLACK)

        val currentPieces = mapOf(
            BoardPosition.of(4, 6) to Spot.PieceSpot(enemyQueen),
            BoardPosition.of(2, 4) to Spot.PieceSpot(enemyQueen),
            BoardPosition.of(6, 2) to Spot.PieceSpot(enemyQueen)
        )
    ...
    fun `given a board with friendly pieces, queen cannot move to or beyond them`() {
        val initialPosition = BoardPosition.of(x = 4, y = 4) // Queen at E5
        val friendlyQueen = Queen(PieceColor.WHITE)

        val currentPieces = mapOf(
            BoardPosition.of(4, 2) to Spot.PieceSpot(friendlyQueen),
            BoardPosition.of(7, 4) to Spot.PieceSpot(friendlyQueen),
            BoardPosition.of(2, 6) to Spot.PieceSpot(friendlyQueen)
        )
    ...
    fun `given a complex board with mixed pieces, queen moves correctly`() {
        ...
        val currentPieces = mapOf(
            // Friendly pieces
            BoardPosition.of(3, 1) to Spot.PieceSpot(Queen(PieceColor.WHITE)), // D2 - blocks down
            BoardPosition.of(5, 5) to Spot.PieceSpot(Queen(PieceColor.WHITE)), // F6 - blocks up-right diagonal

            // Enemy pieces
            BoardPosition.of(0, 3) to Spot.PieceSpot(Queen(PieceColor.BLACK)), // A4 - can capture left
            BoardPosition.of(3, 6) to Spot.PieceSpot(Queen(PieceColor.BLACK)), // D7 - can capture up
            BoardPosition.of(1, 5) to Spot.PieceSpot(Queen(PieceColor.BLACK))  // B6 - can capture up-left diagonal
        )
        ...
        assertThat(result).contains(
            ...
            BoardPosition.of(4, 3), BoardPosition.of(5, 3), BoardPosition.of(6, 3), BoardPosition.of(7, 3),
            // Left horizontal (until enemy piece)
            BoardPosition.of(2, 3), BoardPosition.of(1, 3), BoardPosition.of(0, 3), // Can capture at A4
            // Up vertical (until enemy piece)
            BoardPosition.of(3, 4), BoardPosition.of(3, 5), BoardPosition.of(3, 6), // Can capture at D7
            // Down vertical (until friendly piece)
            BoardPosition.of(3, 2), // Can move until D2
            // Down-left diagonal (unblocked)
            BoardPosition.of(2, 2), BoardPosition.of(1, 1), BoardPosition.of(0, 0),
            // Down-right diagonal (unblocked)
            BoardPosition.of(4, 2), BoardPosition.of(5, 1), BoardPosition.of(6, 0),
            // Up-left diagonal (until enemy piece)
            BoardPosition.of(2, 4), BoardPosition.of(1, 5), // Can capture at B6
    ...
    fun `given a board with pieces surrounding queen, queen can only capture enemy pieces`() {
        ...
        val currentPieces = mapOf(
            ...
            BoardPosition.of(3, 3) to Spot.PieceSpot(Queen(PieceColor.WHITE)), // Down-left
            BoardPosition.of(4, 3) to Spot.PieceSpot(Queen(PieceColor.WHITE)), // Down
            BoardPosition.of(5, 5) to Spot.PieceSpot(Queen(PieceColor.WHITE)), // Up-right
            BoardPosition.of(3, 5) to Spot.PieceSpot(Queen(PieceColor.WHITE)), // Up-left

            // Enemy pieces (4 directions)
            BoardPosition.of(5, 3) to Spot.PieceSpot(Queen(PieceColor.BLACK)), // Down-right
            BoardPosition.of(5, 4) to Spot.PieceSpot(Queen(PieceColor.BLACK)), // Right
            BoardPosition.of(4, 5) to Spot.PieceSpot(Queen(PieceColor.BLACK)), // Up
            BoardPosition.of(3, 4) to Spot.PieceSpot(Queen(PieceColor.BLACK))  // Left
        )
